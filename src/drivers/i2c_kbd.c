...
static int translate(uint8_t state, uint8_t keycode) {
  ...
  if (keycode >= 0x20 && keycode <= 0x7E) return keycode;
  ...
  switch (keycode) {
    ...
    /* Arrow keys → VT100 */
    case KEY_UP:
      return buf_seq("\033[A");
    case KEY_DOWN:
      return buf_seq("\033[B");
    case KEY_RIGHT:
      return buf_seq("\033[C");
    case KEY_LEFT:
      return buf_seq("\033[D");

    /* Navigation → xterm sequences */
    case KEY_HOME:
      return buf_seq("\033[H");
    case KEY_END:
      return buf_seq("\033[F");
    case KEY_INSERT:
      return buf_seq("\033[2~");
    case KEY_DEL:
      return buf_seq("\033[3~");
    case KEY_PAGE_UP:
      return buf_seq("\033[5~");
    case KEY_PAGE_DOWN:
      return buf_seq("\033[6~");

    /* Function keys → xterm sequences */
    case KEY_F1:
      return buf_seq("\033OP");
    case KEY_F2:
      return buf_seq("\033OQ");
    case KEY_F3:
      return buf_seq("\033OR");
    case KEY_F4:
      return buf_seq("\033OS");
    case KEY_F5:
      return buf_seq("\033[15~");
    case KEY_F6:
      return buf_seq("\033[17~");
    case KEY_F7:
      return buf_seq("\033[18~");
    case KEY_F8:
      return buf_seq("\033[19~");
    case KEY_F9:
      return buf_seq("\033[20~");
    case KEY_F10:
      return buf_seq("\033[21~");
...
void kbd_init(void) {
  ...
  for (int i = 0; i < 16; i++) {
    uint8_t status = 0;
    if (i2c_read_reg(KBD_ADDR, REG_ID_KEY, &status, 1) < 0) break;
    if ((status & 0x1F) == 0) break;
    uint8_t fifo[2];
...
int kbd_poll_avail(void) {
  ...
  uint8_t status = 0;
  if (i2c_read_reg(KBD_ADDR, REG_ID_KEY, &status, 1) < 0) return 0;
  return (status & 0x1F) != 0;
}

int kbd_poll(void) {
  /* Return buffered escape sequence bytes first */
  if (seq_pos < seq_len) return (uint8_t)seq_buf[seq_pos++];
  ...
  /* Check FIFO count */
  uint8_t status = 0;
  if (i2c_read_reg(KBD_ADDR, REG_ID_KEY, &status, 1) < 0) return -1;
  ...
  uint8_t fifo[2] = {0, 0};
  if (i2c_read_reg(KBD_ADDR, REG_ID_FIF, fifo, 2) < 0) return -1;
