/*
 * stage2_core.c — PPAP X68000 Stage 2: device-agnostic 44bsd UFS kernel loader
 *
 * Runs at 0x003000 after stage1.  Parses the 44bsd UFS that a device backend
 * (stage2_floppy.c / stage2_scsi.c) exposes through read_ufs_block(), loads
 * /boot/kernel to 0x006000, copies the kernel vector table to 0x000000
 * (restoring the IPL ROM handlers), and jumps to Reset_Handler at 0x006400.
 * The same UFS is later mounted live as the rootfs, so there is no in-RAM
 * rootfs image and no second (outer) UFS layer.
 *
 ...
 *
 * Endianness: mkufs is invoked with -B (big-endian) so on-disk fields read
 * natively on m68k.  Inodes are accessed via a packed struct.
 */
...
/* Scratch buffer: one UFS block (4096 bytes) at the first free address above
 * stage2 code.  $003C00 is the first byte after stage2's 3 KB code area
 * ($003000-$003BFF) and is above all system work areas (vectors, IOCS, OPM,
 * FDC).  See stage2_head.S for the SSP choice. */
...
static uint32_t
    g_iblkno_frag; /* fs_iblkno from SB — first frag of inode table */
...
static uint32_t load_file(const ufs_inode_t *inode, uint8_t *dest) {
  /* Copy fields to locals before any further disk I/O: the IOCS read
   * handlers use the supervisor stack heavily and can clobber the caller's
   * inode struct if it shares the stack. */
...
#pragma GCC diagnostic ignored "-Warray-bounds"
   ...
   * of those so floppy / SCSI / keyboard / timers keep working. */
  uint32_t iocs_handler = *(volatile uint32_t *)BOOT_HANDOFF_IOCS_SAVE;
...
void stage2_main(void) {
   ...
   * device read — putc into the IOCS work area can confuse the FDC handler. */
  ...
  /* Handoff: record the medium we booted from so the kernel mounts the
   * matching rootfs (fd0/sd0).  No rootfs address/size — the kernel mounts
   * the same UFS live via its block driver. */
  *(volatile uint32_t *)BOOT_HANDOFF_MAGIC_ADDR = BOOT_HANDOFF_MAGIC;
  *(volatile uint32_t *)BOOT_HANDOFF_DEV_ADDR = stage2_boot_device;
