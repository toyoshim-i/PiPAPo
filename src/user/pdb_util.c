...
static int append_script_cmd(char **script_cmds, int *script_count,
  ...
  if (*script_count >= PDB_SCRIPT_CMD_MAX) return -1;
  while (cmd[len]) len++;
  if (*storage_used + len + 1 > PDB_SCRIPT_BUF_MAX) return -1;

  for (int i = 0; i < len; i++) storage[*storage_used + i] = cmd[i];
...
int is_script_space(char c) { return c == ' ' || c == '\t'; }

static int append_script_line(char **script_cmds, int *script_count,
                              char *storage, int *storage_used, char *line,
                              int len) {
  int start = 0;
  int end = len;

  while (start < len && is_script_space(line[start])) start++;
  while (end > start && is_script_space(line[end - 1])) end--;
  if (end <= start) return 0;       /* blank line */
  if (line[start] == '#') return 0; /* comment line */

  line[end] = '\0';
  return append_script_cmd(script_cmds, script_count, storage, storage_used,
                           &line[start]);
}
...
int load_script_file(const char *path, char **script_cmds, int *script_count,
  ...
  for (;;) {
    ...
    if (c == '\n') {
      ...
      if (append_script_line(script_cmds, script_count, storage, storage_used,
                             line, len) < 0) {
  ...
  if (append_script_line(script_cmds, script_count, storage, storage_used, line,
                         len) < 0) {
    put_err("pdb: script command limit exceeded while reading ");
    put_err(path);
    put_chr('\n');
...
int parse_x_spec(const char *tok0, uint32_t *count_out, char *fmt_out) {
  ...
  if (!tok0 || tok0[0] != 'x' || tok0[1] != '/') return 0;
...
int parse_mem_width(const char *tok, uint32_t *width_out) {
  if (!tok || !*tok || streq(tok, "w") || streq(tok, "word") ||
      streq(tok, "4")) {
    *width_out = 4u;
    return 1;
  }
