...
static void pl_puts(int fd, const char *s) { write(fd, s, pl_strlen(s)); }
...
static int term_cols(void) {
  struct winsize ws;
  if (ioctl(0, TIOCGWINSZ, &ws) == 0 && ws.ws_col > 0) return ws.ws_col;
  return 80;
}
...
static void erase_eol(void) { write(1, "\033[K", 3); }
 ...
 * "bar"     → dir=".", prefix="bar"
 */
static void comp_split(const char *word, int wlen, char *dir, int dir_size,
  ...
  if (last_slash >= 0) {
    ...
    pl_strncpy(prefix, word + last_slash + 1, wlen - last_slash - 1,
               prefix_size);
  } else {
    dir[0] = '.';
    dir[1] = '\0';
...
static int comp_insert(char *buf, int len, int pos, int *pos_out,
  ...
  for (int i = len; i >= pos; i--) buf[i + total] = buf[i];
  for (int i = 0; i < tlen; i++) buf[pos + i] = text[i];
  buf[pos + tlen] = suffix;
  len += total;
  *pos_out = pos + total;
  buf[len] = '\0';
 ...
 */
static void cmd_scan(const char *prefix, int plen, char *first_match,
                     char *common, int *match_count) {
  /* 1. Builtins */
  ...
  while (*p) {
    ...
    if (dfd >= 0) {
      ...
      while (getdents(dfd, &de, sizeof(de)) > 0) {
        ...
        if (*match_count > 0 && pl_streq(de.d_name, first_match)) continue;
...
static void file_scan(const char *dir, const char *prefix, int plen,
  ...
  while (getdents(dfd, &de, sizeof(de)) > 0) {
    ...
    if (*match_count == 0) *first_is_dir = (de.d_type == DT_DIR);
    comp_record(de.d_name, first_match, common, match_count);
  }
  close(dfd);
}
...
static int do_complete(char *buf, int len, int pos, int *pos_out,
  ...
  if (cmd_mode) {
    ...
    if (match_count == 1) {
      ...
      return comp_insert(buf, len, pos, pos_out, first_match + plen,
                         pl_strlen(first_match) - plen, ' ', prompt,
                         prompt_len);
    }
    ...
    if (extra > 0) {
      ...
      for (int i = len; i >= pos; i--) buf[i + extra] = buf[i];
      for (int i = 0; i < extra; i++) buf[pos + i] = common[plen + i];
      len += extra;
      *pos_out = pos + extra;
      buf[len] = '\0';
      refresh_line(prompt, buf, len, *pos_out, prompt_len);
  ...
  char dir[PATH_BUF];
  char prefix[PPAP_NAME_MAX + 1];
  comp_split(buf + wstart, wlen, dir, sizeof(dir), prefix, sizeof(prefix));
  int plen = pl_strlen(prefix);
  int first_is_dir = 0;
  ...
  if (match_count == 1) {
    return comp_insert(buf, len, pos, pos_out, first_match + plen,
                       pl_strlen(first_match) - plen, first_is_dir ? '/' : ' ',
                       prompt, prompt_len);
  }
