...
static int handle_info_break_cmd(pdb_dispatch_ctx_t *ctx, char **tok, int ntok)
{
    pdb_local_bp_t *local_bp = ctx->local_bp;
...
int handle_breakpoint_commands(pdb_dispatch_ctx_t *ctx, char **tok, int ntok)
{
    ...
    if (streq(tok[0], "disable"))
        return handle_disable_cmd(ctx, tok, ntok);
    if (streq(tok[0], "enable"))
        return handle_enable_cmd(ctx, tok, ntok);
    if (streq(tok[0], "delete") || streq(tok[0], "d"))
        return handle_delete_cmd(ctx, tok, ntok);
    if (streq(tok[0], "info"))
        return handle_info_break_cmd(ctx, tok, ntok);
    return 0;
}
