...
static int handle_show_commands(pdb_dispatch_ctx_t *ctx, char **tok, int ntok) {
...

  {
    ...
    if (streq(show_item, "caps")) {
      if (!get_caps_if_stopped(pid, child_stopped, caps)) return 1;
      print_caps(caps);
      return 1;
    }
    if (streq(show_item, "regset")) {
      struct ppap_ptrace_regs regs;
      if (!get_regs_if_stopped(pid, child_stopped, &regs)) return 1;
      put_str("regset=");
...
static int handle_memory_view_commands(pdb_dispatch_ctx_t *ctx, char **tok,
                                       int ntok) {
  pid_t pid = ctx->pid;
  int child_stopped = *ctx->child_stopped;
...
static int handle_disassemble_command(pdb_dispatch_ctx_t *ctx, char **tok,
                                      int ntok) {
  pid_t pid = ctx->pid;
  int child_stopped = *ctx->child_stopped;

  if (!streq(tok[0], "disas")) return 0;

  {
    ...
    if (!regset_pc_index(regs.regset, &pc_idx)) {
      put_err(
          "pdb: disas currently supports arm, z80, and m68k tracees only\n");
      return 1;
    }
...
int handle_inspect_commands(pdb_dispatch_ctx_t *ctx, char **tok, int ntok) {
  ...
  if (handle_show_commands(ctx, tok, ntok)) return 1;
  ...
  if (handle_memory_view_commands(ctx, tok, ntok)) return 1;

  if (handle_disassemble_command(ctx, tok, ntok)) return 1;
