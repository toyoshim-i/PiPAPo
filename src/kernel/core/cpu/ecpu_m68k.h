...
#ifndef PPAP_KERNEL_CORE_CPU_ECPU_M68K_H
#define PPAP_KERNEL_CORE_CPU_ECPU_M68K_H
...
static inline void m68k_write16(m68k_state_t *cpu, uint32_t addr,
                                uint16_t val) {
  addr &= cpu->mem_size - 1;
...
static inline uint32_t m68k_read32(m68k_state_t *cpu, uint32_t addr) {
  ...
  return ((uint32_t)cpu->memory[addr] << 24) |
         ...
         ((uint32_t)cpu->memory[addr + 2] << 8) | cpu->memory[addr + 3];
}

static inline void m68k_write32(m68k_state_t *cpu, uint32_t addr,
                                uint32_t val) {
  addr &= cpu->mem_size - 1;
...
static inline uint32_t m68k_read_sz(m68k_state_t *cpu, uint32_t addr,
  ...
  switch (size) {
    case M68K_SIZE_BYTE:
      return m68k_read8(cpu, addr);
    case M68K_SIZE_WORD:
      return m68k_read16(cpu, addr);
    case M68K_SIZE_LONG:
      return m68k_read32(cpu, addr);
    default:
      return 0;
  }
}

static inline void m68k_write_sz(m68k_state_t *cpu, uint32_t addr, uint32_t val,
                                 uint8_t size) {
  switch (size) {
    case M68K_SIZE_BYTE:
      m68k_write8(cpu, addr, val);
      break;
    case M68K_SIZE_WORD:
      m68k_write16(cpu, addr, val);
      break;
    case M68K_SIZE_LONG:
      m68k_write32(cpu, addr, val);
      break;
  }
}
...
static inline void m68k_write_d(m68k_state_t *cpu, int reg, uint32_t val,
                                uint8_t size) {
  switch (size) {
    case M68K_SIZE_BYTE:
...
#endif /* PPAP_KERNEL_CORE_CPU_ECPU_M68K_H */
