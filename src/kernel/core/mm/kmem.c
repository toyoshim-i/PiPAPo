 ...
 * size of sizeof(void *) = 4 bytes on a 32-bit target, which is always
 * satisfied by real kernel objects (PCBs, file structs, etc.).
 *
 * kmem_pool_init() threads all objects into the free list by walking the
 * backing storage in obj_size strides and writing the next pointer into each
 * slot. The last slot points to NULL (end of list).
 *
...
uint32_t kmem_free_count(const kmem_pool_t *pool) { return pool->free_count; }
