/*
 * m68k_emu.h — Shared Human68k m68k emulator declarations
 *
...
#ifndef PPAP_KERNEL_CORE_SUBSYS_HUMAN68K_M68K_EMU_H
#define PPAP_KERNEL_CORE_SUBSYS_HUMAN68K_M68K_EMU_H
...
typedef struct {
  ...
  uint32_t heap_next;
  uint32_t block_end;
} m68k_emu_exec_state_t;

/*
 * m68k_emu_trap_handler — Human68k DOS call trap handler for m68k emulator.
 *
 * Intercepts F-line (0xFFxx) illegal instructions and translates them
 * into PPAP syscalls.  ctx must point to m68k_emu_exec_state_t.
 */
int m68k_emu_trap_handler(cpu_state_t *state, int trap_type, uint32_t param,
                          void *ctx);

/*
 * m68k_emu_run_process — kernel-mode entry point for emulated Human68k
 * processes.
 *
...
#endif /* PPAP_KERNEL_CORE_SUBSYS_HUMAN68K_M68K_EMU_H */
