/*
 * cpm_host.c — CP/M subsystem memory image setup (Z80 memory map, BIOS stubs)
 *
...
void cpm_load_com(z80_state_t *cpu, cpm_state_t *cpm, const uint8_t *binary,
  ...
  if (size > CPM_MAX_COM_SIZE) size = CPM_MAX_COM_SIZE;
  ...
  if (cmdline && *cmdline) {
    ...
    cpm_parse_fcb(&cpu->memory[CPM_FCB2_ADDR], (*arg2) ? arg2 : NULL);
  } else {
