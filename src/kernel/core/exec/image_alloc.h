 ...
 *
 * Bridge between the mm layer (mem_region_alloc returns a region_t)
 ...
 * mem_region_alloc; no extra link-time symbol.
 */
...
static inline int image_segment_alloc(proc_image_segment_t *seg,
  ...
  region_t r;
  int rc = mem_region_alloc(mem_class, size, 0u, &r);
  if (rc < 0) return rc;
...
static inline int image_segment_alloc_at(proc_image_segment_t *seg,
  ...
  region_t r;
  int rc = mem_region_alloc_at(mem_class, base, size, 0u, &r);
  if (rc < 0) return rc;
  *seg = proc_image_segment_from_region(mem_class, size, image_flags, r);
  return 0;
}
