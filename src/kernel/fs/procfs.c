...
static int gen_meminfo(char *buf, int bufsiz) {
  ...
  pos = fmt_append_u32(buf, pos, bufsiz,
                       (uint32_t)(USER_PAGES_MAX * PAGE_SIZE) / 1024u);
  pos = fmt_append(buf, pos, bufsiz, " kB\nOomCount:   ");
...
static int gen_mounts(char *buf, int bufsiz) {
  ...
  for (uint32_t i = 0; i < VFS_MOUNT_MAX; i++) {
    ...
    pos = fmt_append(buf, pos, bufsiz, " ");
    /* options */
    pos = fmt_append(buf, pos, bufsiz, (mnt->flags & MNT_RDONLY) ? "ro" : "rw");
...
/* ── /proc/battery — battery voltage and percentage ──────────────────────────
 */

static int (*battery_hw_read)(uint8_t *buf, int len);

void procfs_set_battery(int (*read_fn)(uint8_t *buf, int len)) {
  battery_hw_read = read_fn;
}

static int gen_battery(char *buf, int bufsiz) {
  if (!battery_hw_read) return fmt_append(buf, 0, bufsiz, "not available\n");
...
static const procfs_node_t procfs_nodes[] = {
    {"meminfo", gen_meminfo}, {"version", gen_version},
    {"stat", gen_stat},       {"uptime", gen_uptime},
    {"mounts", gen_mounts},   {"battery", gen_battery},
    {"ecpu", gen_ecpu},       {"subsys", gen_subsys},
};
