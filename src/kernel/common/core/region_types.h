 ...
 *
 * The mm-layer's typed allocator (region_alloc / region_alloc_at)
 * returns the minimum information it owns: a base pointer and the
 * page id when page-backed.  Callers that want a fuller image-segment
 * descriptor (vaddr, OWNED flags, mem_class) build one locally at the
 * call site — those fields are proc/exec-layer concerns, not mm.
 *
 * Lives in common/core/ because mod_core.h pulls it across the module
 * boundary (region_alloc / _free appear in the mod_core vtable on
 * pcxt).
 */
