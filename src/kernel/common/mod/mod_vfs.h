 ...
 *
 * Returns bytes read (0 = EOF), or negative errno.  May block on TTY or
 * pipe via the continuation-blocking sched_switch loop in the backend.
 */
 ...
 */
MOD_FUNC(vfs, int, lookup_parent, const char *, vnode_t **, char *, int)
 ...
 */
MOD_FUNC(vfs, int, mount, const char *, const vfs_ops_t *, uint8_t,
         const void *)
 ...
 * Returns 0 on success, negative errno on failure.
 */
MOD_FUNC(vfs, int, mount_by_fstype, const char *, const char *, const char *,
         long)
