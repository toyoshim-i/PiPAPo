...
#define MOD_DECLARE_BEGIN(name) typedef struct mod_##name##_s {
...
#define MOD_FUNC(mod, ret, func, ...) ret __far (*func)(__VA_ARGS__);
#else
#define MOD_FUNC(mod, ret, func, ...) ret (*func)(__VA_ARGS__);
#endif
...
#define MOD_DEFINE_BEGIN(name) mod_##name##_t mod_##name = {
/* Maps unprefixed field to prefixed real function: .init = vfs_init */
#define MOD_IMPL(mod, func) .func = _MOD_CONCAT(mod, func),
