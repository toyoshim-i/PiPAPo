...
static int sd_read_sector(uint32_t addr, uint8_t *buf) {
  ...
  for (int i = 0; i < SD_DATA_TIMEOUT; i++) {
    ...
    if (tok == SD_TOKEN_DATA) goto got_token;
    if (tok != 0xFFu) {
      sd_cs_high();
...
static int sd_parse_mbr(void) {
  ...
  if (mbr[510] != MBR_SIG_BYTE0 || mbr[511] != MBR_SIG_BYTE1) return -EIO;
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (type == PART_TYPE_FAT32 || type == PART_TYPE_FAT32_LBA) {
      ...
      sd_part_lba = (uint32_t)ent[8] | ((uint32_t)ent[9] << 8) |
                    ((uint32_t)ent[10] << 16) | ((uint32_t)ent[11] << 24);
      sd_part_sectors = (uint32_t)ent[12] | ((uint32_t)ent[13] << 8) |
...
int sd_init(void) {
  ...
  /* 5. ACMD41 — SD_SEND_OP_COND (HCS=1 for SDHC support) */
  {
    ...
    while (attempts-- > 0) {
      ...
      sd_cs_high();
      if (r1 == 0x00u) break;
      if (r1 != R1_IDLE) return -EIO;
      delay_ms(1);
    }
    if (r1 != 0x00u) return -ETIMEDOUT;
  }
