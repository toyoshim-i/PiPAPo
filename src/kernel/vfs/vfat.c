...
static int fat_flush(vfat_sb_t *sb) {
  if (!sb->fat_cache_dirty || sb->fat_cache_sector == 0xFFFFFFFFu) return 0;
  for (uint32_t f = 0; f < sb->num_fats; f++) {
    uint32_t s = sb->fat_start_sector + f * sb->fat_size + sb->fat_cache_sector;
    int rc = write_sector(sb, s, sb->fat_cache);
    if (rc < 0) return rc;
  }
  sb->fat_cache_dirty = 0;
...
static int lfn_extract(const fat_lfn_entry_t *lfn, char *name, int pos) {
  ...
  int count = 0;
  for (int i = 0; i < 13; i++) {
    if (chars[i] == 0x0000u || chars[i] == 0xFFFFu) break;
...
static int vfat_mount(mount_entry_t *mnt, const void *dev_data) {
  ...
  /* Validate BPB fields */
  if (bpb->bytes_per_sector != 512) return -EINVAL;
  if (bpb->sectors_per_cluster == 0) return -EINVAL;
  if (bpb->num_fats == 0) return -EINVAL;
  if (bpb->reserved_sectors == 0) return -EINVAL;
  if (bpb->fat_size_32 == 0) return -EINVAL;
  ...
  if (data_start >= bpb->total_sectors_32) return -EINVAL;
  ...
  while (cluster >= 2 && !FAT_EOC(cluster)) {
    ...
    for (uint32_t s = 0; s < sb->sectors_per_cluster; s++) {
      ...
      for (uint32_t off = 0; off < 512; off += 32) {
        ...
        if (de->name[0] == FAT_DIRENT_END) return -ENOENT;
        ...
        if (de->attr == FAT_ATTR_LONG_NAME) {
          ...
          if (lfn->order & FAT_LFN_LAST_ENTRY) {
            ...
            lfn_len = 0;
            collecting_lfn = 1;
            for (int i = 0; i <= VFS_NAME_MAX; i++) lfn_name[i] = '\0';
          }
          if (collecting_lfn) {
...
static long vfat_read(vnode_t *vn, page_id_t page, uint16_t page_off, size_t n,
  ...
  while (total < n && cluster >= 2 && !FAT_EOC(cluster)) {
    ...
    for (uint32_t s = sec_off; s < sb->sectors_per_cluster && total < n; s++) {
      ...
      mod_core.page_write(page, page_off, &sector_buf[start], (uint16_t)avail);
...
static long vfat_write(vnode_t *vn, page_id_t page, uint16_t page_off, size_t n,
  ...
  while (total < n) {
    ...
    for (uint32_t s = sec_off; s < sb->sectors_per_cluster && total < n; s++) {
      ...
      mod_core.page_read(page, page_off, &sector_buf[start], (uint16_t)avail);
  ...
  while (cluster >= 2 && !FAT_EOC(cluster)) {
    ...
    for (uint32_t s = 0; s < sb->sectors_per_cluster; s++) {
      ...
      for (uint32_t off = 0; off < 512; off += 32) {
        ...
        if (de->name[0] == FAT_DIRENT_END) goto done;
        if (de->name[0] == FAT_DIRENT_FREE) {
          collecting_lfn = 0;
          continue;
        }
        ...
        if (entry_idx >= target_idx) {
          ...
          entries[count].d_type =
              (de->attr & FAT_ATTR_DIRECTORY) ? DT_DIR : DT_REG;
...
static int vfat_create(vnode_t *dir, const char *name, uint32_t mode,
  ...
  while (cluster >= 2 && !FAT_EOC(cluster)) {
    ...
    for (uint32_t s = 0; s < sb->sectors_per_cluster; s++) {
      ...
      for (uint32_t off = 0; off < 512; off += 32) {
        fat_dirent_t *de = (fat_dirent_t *)&sector_buf[off];
        if (de->name[0] == FAT_DIRENT_END || de->name[0] == FAT_DIRENT_FREE) {
    ...
    for (uint32_t s = 0; s < sb->sectors_per_cluster; s++) {
      ...
      for (uint32_t off = 0; off < 512; off += 32) {
        ...
        if (str_eq_ci(dname, name)) {
          ...
          if (de->attr & FAT_ATTR_DIRECTORY) return -EISDIR;
...
static int vfat_lookup_locked(vnode_t *dir, const char *name,
                              vnode_t **result) {
  uint32_t s = spin_lock_irqsave(SPIN_FS);
