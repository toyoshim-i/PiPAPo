/*
 * i2c_rpico.c — I2C1 master driver for RP2040/RP2350 (Synopsys DesignWare
 * DW_apb_i2c)
 *
...
static void i2c_clear_abort(void) {
  if (IC_RAW_INTR_STAT & RAW_INTR_TX_ABRT) (void)IC_CLR_TX_ABRT;
}
...
static int i2c_wait_tx_ready(void) {
  ...
  while (!(IC_STATUS & STATUS_TFNF)) {
    if (IC_RAW_INTR_STAT & RAW_INTR_TX_ABRT) return -1;
...
void i2c_init(void) {
   ...
   *    slave mode disabled */
  IC_CON = CON_MASTER_MODE | CON_SPEED_STD | CON_IC_RESTART_EN |
           CON_IC_SLAVE_DISABLE;
...
int i2c_read_reg(uint8_t addr, uint8_t reg, uint8_t *buf, size_t len) {
  ...
  if (i2c_wait_tx_ready() < 0) goto fail;
  IC_DATA_CMD = reg;
  ...
  for (size_t i = 0; i < len; i++) {
    if (i2c_wait_rx_ready() < 0) goto fail;
