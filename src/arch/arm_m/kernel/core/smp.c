...
#define PSM_FRCE_OFF_SET \
  (*(volatile uint32_t *)0x4001A004u) /* atomic SET alias */
#define PSM_FRCE_OFF_CLR \
  (*(volatile uint32_t *)0x4001B004u) /* atomic CLR alias */
#define PSM_PROC1 (1u << 24)          /* bit 24 = proc1 on RP2350 */
#else
#define PSM_FRCE_OFF (*(volatile uint32_t *)0x40010004u)
#define PSM_FRCE_OFF_SET \
  (*(volatile uint32_t *)0x40012004u) /* atomic SET alias */
#define PSM_FRCE_OFF_CLR \
  (*(volatile uint32_t *)0x40013004u) /* atomic CLR alias */
...
void core1_sched_entry(void) {
  ...
  uint32_t psp_top =
      (uint32_t)(uintptr_t)page_to_ptr(idle->stack_page_id) + PAGE_SIZE;
  __asm__ volatile(
...
static void core1_reset(void) {
  ...
  while (!(SIO_FIFO_ST & SIO_FIFO_VLD)) arch_wfe();
...
void core1_launch(void (*entry)(void)) {
  ...
  while (i < 6) {
    ...
    if (word == 0u) {
      ...
      while (SIO_FIFO_ST & SIO_FIFO_VLD) (void)SIO_FIFO_RD;
      arch_sev();
    }
