/*
 * spi_rpico.c — SPI0 driver for RP2040/RP2350 PL022 (ARM PrimeCell SSP)
 *
...
static void compute_prescaler(uint32_t baud_hz, uint32_t *cpsdvsr,
                              uint32_t *scr) {
...
void spi_init(uint32_t baud_hz) {
  ...
  SPI0_CR0 = CR0_DSS_8BIT | CR0_FRF_SPI | CR0_CPOL_0 | CR0_CPHA_0 | (scr << 8);
...
void sd_cs_low(void) { SIO_GPIO_OUT_CLR = (1u << PICOCALC_SPI0_CS); }
