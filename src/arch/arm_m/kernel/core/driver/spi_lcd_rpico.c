 ...
 * data to the LCD (ST7365P / ILI9488).  Three additional GPIO pins provide
 * chip-select (CS), data/command (DC), and hardware reset (RST).
 *
...
static inline void drain_rx(void) {
  while (SPI1_SR & SR_RNE) (void)SPI1_DR;
}
...
void spi_lcd_data16(const uint16_t *buf, size_t count) {
  ...
  for (size_t i = 0; i < count; i++) {
    uint16_t v = buf[i];
    if (spi1_write_byte((uint8_t)(v >> 8))) {
      cs_high();
      return;
    }
    if (spi1_write_byte((uint8_t)(v & 0xFF))) {
      cs_high();
      return;
    }
    if ((i & 3u) == 3u) drain_rx();
...
void spi_lcd_fill(uint16_t color, uint32_t count) {
  ...
  for (uint32_t i = 0; i < count; i++) {
    if (spi1_write_byte(hi)) {
      cs_high();
      return;
    }
    if (spi1_write_byte(lo)) {
      cs_high();
      return;
    }
    if ((i & 3u) == 3u) drain_rx();
