/*
 * sigaction.c — RISC-V user-space sigaction() wrapper.
 *
 ...
 * handles the ePIC reference to _ppap_sigreturn_trampoline so the
 * runtime value carries the correct load-relative address.
 *
 * Mirrors the ARM and m68k wrappers; same sa_restorer model used by
 * glibc / musl on Linux.
 */
