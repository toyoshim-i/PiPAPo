...
- **`src/etc/`** — config file templates (fstab, passwd, inittab, …) tracked in git
- **`build/arm_m/romfs/`** — built binaries (busybox, rogue, user programs) generated by the build

Per-target staging merges these into `build/arm_m/romfs_<target>/`:

```
romfs_ppap_pico1calc/
  bin/          User binaries (hello, rogue, busybox applet symlinks)
  sbin/         System binaries (init, mount → busybox)
  etc/
    inittab     Per-target init configuration
    profile     Shell startup (PATH, PS1)
    hostname    System hostname
    passwd      User database (root only)
    group       Group database
    fstab       Per-target filesystem mount table
...
1. Place your binary in `build/arm_m/romfs/bin/`:
   ```sh
   cp myapp.elf build/arm_m/romfs/bin/myapp
   ```
...
5. Runs `mkromfs` to generate `romfs_<target>.bin`
6. Links `romfs.bin` into the kernel via `.incbin`
...
# Create image from a staging directory
build/arm_m/mkromfs build/arm_m/romfs_ppap_pico1calc/ build/arm_m/romfs.bin
