...
[archive/history/target-68000-plan.md](/docs/archive/history/target-68000-plan.md).
...
See [subsystems/overview.md](/docs/subsystems/overview.md) for the full subsystem
design including the Human68k personality layer.

### Emulators for Development

- **XEiJ** — Java-based emulator (included in `ppap/m68k` Docker image, requires OpenJDK 25+)
- **XM6 TypeG** — accurate X68000 emulator (Windows, runs under Wine)
- Custom QEMU with X68000 machine support (community patches)

XEiJ is the default emulator for `./scripts/run.sh x68k`. It boots the floppy
image and routes AUX serial to TCP (port 54321) so kernel output appears on stdout.
...
- [archive/history/target-68000-plan.md](/docs/archive/history/target-68000-plan.md) — Original planning document
