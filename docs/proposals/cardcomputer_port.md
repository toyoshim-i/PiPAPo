...
| SPI transport impl | `src/arch/arm_m/.../spi_lcd_rpico.c` (PL022) | new `src/arch/xtensa/kernel/vfs/driver/spi_lcd_xtensa_cc.c` — start on ESP-IDF `spi_master`, lower to bare MMIO at SPI2 base in CC-3.5e |
...
| CC-4b | Implement SPI2 transport in `src/arch/xtensa/kernel/vfs/driver/spi_lcd_xtensa_cc.c`, exporting the existing `spi_lcd.h` API. Phase 1 uses ESP-IDF `spi_master` (queued transactions, DMA-capable buffers); MOSI=35, SCK=36, CS=37, DC=34, RST=33 from `xtensa_cc.h`. Validate with a known-pattern fill before wiring fbcon. |
