/*
 * test_alloc.c — Unit tests for src/user/lib/alloc.c
 *
...
/* Declare the API locally — we can't pull uclib.h wholesale
...
static void test_best_fit_picks_smallest(void) {
  ...
  void *b = malloc(64);
  void *c = malloc(32);
  void *d = malloc(16);  /* keeps the 16-hole from being the last block */
  (void)d;
  ASSERT_NOT_NULL(a); ASSERT_NOT_NULL(b); ASSERT_NOT_NULL(c);
  free(a);  /* free a 16-byte hole */
  free(c);  /* free a 32-byte hole */
  free(b);  /* free a 64-byte hole sandwiched between used d and end
