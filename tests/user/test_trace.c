 ...
 *   2. PTRACE_SYSCALL reports syscall enter/exit events
 *   3. PTRACE_CONT resumes the child to normal exit
 */
...
int main(void)
{
    ...
    UT_ASSERT_EQ(ptrace(PTRACE_SYSCALL, pid, (void *)0, (void *)0), 0);
    UT_ASSERT_EQ(waitpid(pid, &status, WSTOPPED), pid);
