/*
 * ktest.c — Kernel integration test suites
 *
 * Extracted from main_qemu.c.  Only compiled when PPAP_TESTS=ON.
 * Called from target_post_mount() after VFS + fstab mount.
 *
 * Test suites:
 *   vfs, pipe, dup, brk, signal, blkdev, vfat, loopback,
 *   tmpfs, ufs, fstab, blocking_io
 */
...
static void vfs_integration_test(void)
{
    ...
    /* 1. open + read /etc/hostname via syscall layer */
    {
        ...
        if (fd >= 0) {
            char buf[32];
            long n = ktest_sys_read(fd, buf, sizeof(buf) - 1);
            if (n == 5) {
                buf[n] = '\0';
                /* "ppap\n" */
    ...
    /* 4. open + write /dev/null */
    {
        ...
        int ok = 0;
        if (fd >= 0) {
            long n = ktest_sys_write(fd, "discarded", 9);
            ok = (n == 9);
            sys_close(fd);
        }
    ...
    /* 5. open + read /dev/zero → all zero bytes */
    {
        ...
        if (fd >= 0) {
            ...
            long n = ktest_sys_read(fd, buf, 4);
            if (n == 4)
...
static void pipe_integration_test(void)
{
    ...
    /* 1. pipe + write + read */
    {
        ...
        int ok = 0;
        if (rc == 0) {
            long nw = ktest_sys_write(fds[1], "hello", 5);
            if (nw == 5) {
                char buf[8] = {0};
                long nr = ktest_sys_read(fds[0], buf, sizeof(buf));
                if (nr == 5)
    ...
    /* 3. pipe EPIPE: close read end, write → returns -EPIPE */
    {
        ...
        if (rc == 0) {
            ...
            long nw = ktest_sys_write(fds[1], "x", 1);
            ok = (nw == -(long)EPIPE);
            sys_close(fds[1]);
        }
    ...
    /* 4. pipe partial fill: fill buffer, verify partial write count */
    {
        ...
        if (rc == 0) {
            ...
            long n1 = ktest_sys_write(fds[1], wbuf, 256);
            long n2 = ktest_sys_write(fds[1], wbuf, 256);
            /* n1=256, n2=255 (only 255 bytes of space left) */
            ok = (n1 == 256 && n2 == 255);

            /* Drain and verify first byte */
            char rbuf[512];
            long nr = ktest_sys_read(fds[0], rbuf, sizeof(rbuf));
            if (nr == 511)
                ok = ok && (rbuf[0] == 0);
...
static void dup_integration_test(void)
{
    ...
    /* 1. dup basic: pipe, dup read-end, read from dup'd fd */
    {
        ...
        if (rc == 0) {
            ...
            if (dup_fd >= 0) {
                ktest_sys_write(fds[1], "dup!", 4);
                char buf[8] = {0};
                long nr = ktest_sys_read(dup_fd, buf, sizeof(buf));
    ...
    /* 4. dup2 closes target: dup2 over an open pipe fd */
    {
        ...
        if (rca == 0 && rcb == 0) {
            ...
            long nw = ktest_sys_write(fds_b[1], "x", 1);
            ok = (nw == -(long)EPIPE);
            sys_close(fds_a[0]);
            sys_close(fds_a[1]);
...
static void blkdev_integration_test(void)
{
    ...
    /* 1. blkdev_init + register: ramblk already called from target_late_init */
    {
...
static void vfat_integration_test(void)
{
    ...
    /* 2. Read /mnt/sd/data.bin (256 bytes: 0x00..0xFF) */
    {
        ...
        if (fd >= 0) {
            char buf[256];
            long n = ktest_sys_read(fd, buf, 256);
            if (n == 256) {
                ok = 1;
                for (int i = 0; i < 256; i++) {
    ...
    /* 6. Create + write + read a new file */
    {
        ...
        int ok = 0;
        if (fd >= 0) {
            long nw = ktest_sys_write(fd, "test data", 9);
            sys_close(fd);
            if (nw == 9) {
                /* Read it back */
                ...
                if (fd >= 0) {
                    char buf[16] = {0};
                    long nr = ktest_sys_read(fd, buf, sizeof(buf));
...
static void tmpfs_integration_test(void)
{
    ...
    /* 2. Create a file, write, close, re-open, read back */
    {
        ...
        int ok = (fd >= 0);
        if (ok) {
            long n = ktest_sys_write(fd, "hello", 5);
            ok = (n == 5);
            sys_close(fd);
        }
        if (ok) {
            ...
            if (fd >= 0) {
                char buf[16];
                long n = ktest_sys_read(fd, buf, 16);
    ...
    /* 5. mkdir + nested file */
    {
        ...
        if (ok) {
            ...
            if (fd >= 0) {
                char buf[8];
                long n = ktest_sys_read(fd, buf, 8);
                ok = (n == 4 && buf[0] == 'n');
                sys_close(fd);
            } else {
                ok = 0;
    ...
    /* 7. Write exceeds TMPFS_DATA_MAX → ENOSPC */
    {
        ...
        if (fd1 >= 0 && fd2 >= 0) {
            ...
            long n1 = ktest_sys_write(fd1, buf, 64);  /* allocates page 2 of 2 */
            long n2 = ktest_sys_write(fd2, buf, 64);  /* 3rd page → ENOSPC */
    ...
    /* 8. Unlink reclaims page, write succeeds */
    {
        ...
        if (fd >= 0) {
            ...
            long n = ktest_sys_write(fd, buf, 32);
            ok = (n == 32);
            sys_close(fd);
        }
...
static void ufs_integration_test(void)
{
    ...
    /* 1. Create + write + read back */
    {
        ...
        int ok = 0;
        if (fd >= 0) {
            long n = ktest_sys_write(fd, "test data\n", 10);
            sys_close(fd);
            if (n == 10) {
                fd = sys_open("/mnt/ufs/newfile.txt", O_RDONLY, 0);
                if (fd >= 0) {
                    char buf[16];
                    long r = ktest_sys_read(fd, buf, sizeof(buf));
    ...
    /* 4. Write after truncate + read back */
    {
        ...
        int ok = 0;
        if (fd >= 0) {
            long n = ktest_sys_write(fd, "rewritten\n", 10);
            sys_close(fd);
            if (n == 10) {
                fd = sys_open("/mnt/ufs/newfile.txt", O_RDONLY, 0);
                if (fd >= 0) {
